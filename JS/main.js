...
//     const specularFactor = Math.pow(Math.max(0, viewDir.dot(reflectDir)), Math.max(1, 30)); // assuming shininess 50
...
function rayColour(ray)
{
    ...
    return new Vec3(Math.min(1, colour.x), Math.min(1, colour.y), Math.min(1, colour.z));
}
